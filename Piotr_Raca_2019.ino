...
void loop() {
  ...
  // regulator typu P
  if (a == 0 && b == 0 && c == 0 && gotowa == false) {
    gotowa = true;
    Serial.println("Done!");
  }

  int temp = analogRead(TEMP_0_PIN);
  double x = (double) temp;
  double y = A * x * x * x + B * x * x + C * x + D;

  double blad = nastawa - y;
  int P;
  int k = 7;

  if (blad > 0)
  {
    P = map(blad, 0, nastawa, 0, 255);
    P = P * k;
    P = P < 255 ? P : 255;
    analogWrite(HEATER_0_PIN, P);
    lcd.setCursor(0, 3);
    lcd.print("Heater: ");
    lcd.print(P);
  }
